...
public enum NpcSideFloresta { Esquerda, Direita }

[System.Serializable]
public class DialogLineFloresta
{
    public string speakerName = "Capitão Cenoura";
    [TextArea(2, 4)] public string text;
    public NpcSideFloresta npcQueFala = NpcSideFloresta.Esquerda; // Escolhe qual NPC fala nesta linha
}

public class DialogManagerFloresta : MonoBehaviour
{
    [Header("NPC Esquerda")]
    ...
    [Header("Diálogos")]
    [SerializeField] private DialogLineFloresta[] lines;
    ...
    public void SetSingleLine(string speaker, string text, NpcSideFloresta side = NpcSideFloresta.Esquerda)
    {
        lines = new DialogLineFloresta[]
        {
            new DialogLineFloresta { speakerName = speaker, text = text, npcQueFala = side }
        };
    }
    ...
    private void DestaqueNpcAtivo(NpcSideFloresta ladoAtivo)
    {
        // 1.0f para opacidade total (destacado), 0.5f para meio opaco (escondido/fundo)
        if (npcLeftCanvasGroup != null) npcLeftCanvasGroup.alpha = (ladoAtivo == NpcSideFloresta.Esquerda) ? 1.0f : 0.5f;
        if (npcRightCanvasGroup != null) npcRightCanvasGroup.alpha = (ladoAtivo == NpcSideFloresta.Direita) ? 1.0f : 0.5f;
    }
