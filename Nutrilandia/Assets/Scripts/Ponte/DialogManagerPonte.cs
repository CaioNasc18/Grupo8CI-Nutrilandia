...
public enum NpcSidePonte { Esquerda, Direita }

[System.Serializable]
public class DialogLinePonte
{
    public string speakerName = "Capitão Cenoura";
    [TextArea(2, 4)] public string text;
    public NpcSidePonte npcQueFala = NpcSidePonte.Esquerda; // Escolhe qual NPC fala nesta linha
}

public class DialogManagerPonte : MonoBehaviour
{
    [Header("NPC Esquerda")]
    ...
    [Header("Diálogos")]
    [SerializeField] private DialogLinePonte[] lines;
    ...
    public void SetSingleLine(string speaker, string text, NpcSidePonte side = NpcSidePonte.Esquerda)
    {
        lines = new DialogLinePonte[]
        {
            new DialogLinePonte { speakerName = speaker, text = text, npcQueFala = side }
        };
    }
    ...
    private void DestaqueNpcAtivo(NpcSidePonte ladoAtivo)
    {
        // 1.0f para opacidade total (destacado), 0.5f para meio opaco (escondido/fundo)
        if (npcLeftCanvasGroup != null) npcLeftCanvasGroup.alpha = (ladoAtivo == NpcSidePonte.Esquerda) ? 1.0f : 0.5f;
        if (npcRightCanvasGroup != null) npcRightCanvasGroup.alpha = (ladoAtivo == NpcSidePonte.Direita) ? 1.0f : 0.5f;
    }
